...
exports.cancelEvent = catchAsync(async (req, res, next) => {
   ...
   if (!event){
      ...
      if(event.booked < event.capacity){
         ...
         // //update the guy that cancel order to canceled order
         // let iscancelOrder =  await database.orders.update({status: 'canceled' }, { where: { id:order.id, userId:req.body.userId } });
         // console.log("*******iscancelOrder ", iscancelOrder);
